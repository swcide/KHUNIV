...
//	-------------------------- 학사공지 ------------------------------
	...
	public ModelAndView noticeList(ModelAndView mv, HttpSession session, @RequestParam int nType, @RequestParam(value = "currentPage", required = false, defaultValue = "1") int currentPage) {
	...
	public ModelAndView noticeDetail(ModelAndView mv, Notice n, @RequestParam(value = "currentPage", required = false, defaultValue = "1") int currentPage) {
		...
		if (n.getnType() == 1) {
			mv.addObject("n", n).addObject("currentPage", currentPage).setViewName("notice/dept_notice_detail");

		} else if (n.getnType() == 2) {

			mv.addObject("n", n).addObject("currentPage", currentPage).setViewName("notice/gener_notice_detail");
